...
public class _29_PaintJob {
    public static void main(String[] args) {
        System.out.println(getBucketCount(2.75, 3.25, 2.5, 1));
    }

    public static int getBucketCount(double w, double h, double apb, int extraBuckets) {
        if (w <= 0 || h <= 0 || apb <= 0 || extraBuckets < 0)
            return -1;
        double area = w * h;
        int count;
        count = (int) (Math.ceil((area / apb) - extraBuckets));
        return count;
    }

    public static int getBucketCount(double width, double height, double areaPB) {
        if (width <= 0 || height <= 0 || areaPB <= 0)
            return -1;
        double area = width * height;
        int count;
        count = (int) (Math.ceil((area / areaPB)));
        return count;
    }
